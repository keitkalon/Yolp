        ...
        for comment in comments:
            if comment["id"] == comment_id:
                return render_template(
                    "edit_comment.html", comment=comment, comment_id=comment_id
                )
...
def edit_answer_page(answer_id):
    ...
    if request.method == "POST":
        ...
        if request.files["image"].filename != "":
            ...
            if not myutility.allowed_image_files(
                image.filename, app.config["ALLOWED_IMAGE_EXTENSION"]
            ):
    ...
    if request.method == "GET":
        ...
        for answer in answers:
            if answer["id"] == answer_id:
                return render_template(
                    "edit_answer.html", answer=answer, answer_id=answer_id
                )
    ...
    if request.method == "POST":
        ...
        for question in questions:
            if question["id"] == question_id:
                return render_template(
                    "edit_question.html", question=question, question_id=question_id
                )
    ...
    if request.method == "POST":
        ...
        if request.files["image"].filename != "":
            ...
            if image:
                ...
                image.save(os.path.join(app.config["IMAGE_UPLOADS"], image.filename))
        dict_answers = myutility.init_answer_and_question(
            new_answer, "a", image_filename, question_id
        )
        ...
        return render_template(
            ...
            answers=show_answer,
            comments_question=show_comments_questions,
            comments_answers=show_comments_answers,
            tags=show_tags,
        )
